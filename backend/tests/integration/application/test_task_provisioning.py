...
from crucible.application.errors import RevisionNotFound
...
async def test_task_provisioning_keeps_registered_checkout_unchanged(
    ...
    task = await service.create(registered.repository.id, "HEAD")
...
async def test_unresolved_ref_evidence_blocks_lossy_downgrade(
    ...
    with pytest.raises(RevisionNotFound):
        await service.create(registered.repository.id, "missing")
